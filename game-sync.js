        ...
        if (results.length > 0) {
            const team = results.map(p => {
                ...
                return {
                    ...
                    maxHp: p.max_hp,
                    border: border,
                    image: p.image || `https://cdn.jsdelivr.net/gh/PokeAPI/sprites@master/sprites/pokemon/other/official-artwork/${p.pokedex_number}.png`,
                    moves: p.moves || []
                };
            });
...
async function syncAdminDataFromBackend() {
    try {
        ...
        if (statsData && statsData.stats) {
            const statBlocks = document.querySelectorAll(".metric-card h2");
            // If they are on dashboard, update metrics
            if (statBlocks.length >= 4) {
                statBlocks[0].textContent = statsData.stats.totalUsers;
                statBlocks[1].textContent = statsData.stats.totalCapturedPokemon;
                statBlocks[2].textContent = statsData.stats.totalBattles;
                statBlocks[3].textContent = statsData.stats.activeUsersToday;
            }
        }
            ...
            const wrappedCallback = async function() {
                ...
                try {
                    if (title.includes("Pokémon")) {
                        ...
                        await syncApiRequest("/missions/", "POST", payload); // Note: Admin CRUD uses default missions endpoint
        ...
        window.deletePokemon = async function(idx) {
            ...
            if (target && target.id) {
                try {
                    ...
                    showToast("Failed to delete Pokémon from database.");
                }
            }
            originalDelete(idx);
        };
